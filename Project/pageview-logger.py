...
class PageView:
    ...
    def to_json(self):
        return json.dumps({"test-topic1": {"url": self.url}})
...
class PageViewKafkaLogger:
    ...
    def produce(self, pageview: PageView):
        try:
            future = self.producer.send(topic='test-topic1', key=f"{random.randrange(999999)}".encode(), value=pageview.to_json().encode())
